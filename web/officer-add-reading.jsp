...
<head>
    ...
    <title>Tambah Bacaan - HydroAlert</title>
...
<aside class="sidebar">
    ...
    <nav class="sb-nav">
        ...
        <a href="officer-add-reading.jsp" class="sb-link active"><i class="fa-solid fa-plus-circle"></i> Add Reading</a>
...
<div class="main">
<div class="topbar">
    <div class="topbar-title">Tambah Bacaan</div>
    <div class="topbar-user"><div class="tb-avatar officer"><i class="fa-solid fa-user-shield"></i></div><%= officerName %></div>
</div>
<div class="body-content">
    <div class="sec-header">
        <div class="sec-title"><i class="fa-solid fa-plus-circle"></i> Tambah Bacaan Banjir Baru</div>
        <a href="officer-dashboard.jsp" class="btn-cancel" style="padding: 8px 16px;"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
    </div>

    <% if(request.getParameter("success") != null) { %>
        <div class="alert alert-success"><i class="fa-solid fa-circle-check"></i> Rekod bacaan berjaya ditambahkan!</div>
    <% } %>
    <% if(request.getParameter("error") != null) { %>
        <div class="alert alert-error"><i class="fa-solid fa-triangle-exclamation"></i> Gagal merekod bacaan baru. Sila cuba lagi.</div>
    <% } %>

    <div class="form-card">
        <form action="AddReadingServlet" method="post" onsubmit="return prepareSubmit()">
            
            <!-- STATION NAME -->
            <div class="form-group">
                <label class="form-label">Nama Stesen *</label>
                <div id="stationDropdownWrap">
                    <select id="stationDropdown" name="_stationDropdown" onchange="onStationSelect(this)" class="form-input" style="width: 100%;">
                        <option value="">-- Pilih stesen sedia ada --</option>
                        <%
                            ...
                            try {
                                ...
                                pstmt = conn.prepareStatement(
                                    "SELECT DISTINCT station_name, location, state, latitude, longitude " +
                                    "FROM readings ORDER BY state, station_name");
                                rs = pstmt.executeQuery();
                                while(rs.next()) {
                                    String sName = rs.getString("station_name");
                                    String sLoc  = rs.getString("location");
                                    String sSt   = rs.getString("state");
                                    double sLat  = rs.getDouble("latitude");
                                    boolean hasLat = !rs.wasNull();
                                    double sLng  = rs.getDouble("longitude");
                                    boolean hasLng = !rs.wasNull();
                                    String latVal = hasLat ? String.valueOf(sLat) : "";
                                    String lngVal = hasLng ? String.valueOf(sLng) : "";
                        %>
                        <option value="<%= sName %>"
                                data-location="<%= sLoc %>"
                                data-state="<%= sSt %>"
                    ...
                    <span class="station-toggle" onclick="toggleManualStation()">
                        <i class="fa-solid fa-pen-to-square"></i> Masukkan nama stesen secara manual
                    </span>
                </div>

                <div id="stationManualWrap" style="display:none;">
                    <input type="text" id="stationManualInput" class="form-input" placeholder="Taip nama stesen secara manual" style="width: 100%;">
                    <span class="station-toggle" onclick="toggleManualStation()">
                        <i class="fa-solid fa-list"></i> Pilih daripada senarai stesen sedia ada
                    </span>
                </div>

                <input type="hidden" id="stationNameFinal" name="stationName">
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="locationField" name="location" class="form-input" required placeholder="Diisi secara automatik apabila stesen dipilih">
                </div>
                <div class="form-group">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="waterLevelInput" name="waterLevel" class="form-input" step="0.01" placeholder="cth: 2.50" oninput="autoRiskFromWaterLevel(this.value)">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="rainfallInput" name="rainfall" class="form-input" step="0.01" placeholder="cth: 12.0" oninput="autoTrendFromRainfall(this.value)">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="number" id="latField" name="latitude" class="form-input" step="0.000001" placeholder="cth: 3.139003" oninput="onCoordsInput()">
                    <span style="font-size:11px;color:var(--muted);margin-top:2px;">Diisi automatik jika stesen mempunyai koordinat</span>
                </div>
                <div class="form-group">
                    <label class="form-label">Longitud</label>
                    <input type="number" id="lngField" name="longitude" class="form-input" step="0.000001" placeholder="cth: 101.686855" oninput="onCoordsInput()">
                    <span style="font-size:11px;color:var(--muted);margin-top:2px;">Diisi automatik jika stesen mempunyai koordinat</span>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="notes" class="form-input" rows="3" placeholder="Maklumat tambahan atau ulasan tentang bacaan stesen..."></textarea>
            </div>

            <div class="form-actions">
                <button type="submit" class="btn-submit"><i class="fa-solid fa-circle-check"></i> Tambah Bacaan</button>
                <a href="officer-dashboard.jsp" class="btn-cancel">Batal</a>
            </div>
        </form>
    </div>
</div>
...
function onStationSelect(sel) {
    ...
    if(!opt.value) return;
    document.getElementById('locationField').value = opt.getAttribute('data-location') || '';
    ...
    document.getElementById('stationNameFinal').value = opt.value;
}

function setStateDropdown(stateVal) {
...
function toggleManualStation() {
    isManual = !isManual;
    document.getElementById('stationDropdownWrap').style.display = isManual ? 'none' : 'block';
    document.getElementById('stationManualWrap').style.display   = isManual ? 'block' : 'none';
    document.getElementById('stationNameFinal').value = '';
    if(!isManual) {
...
function prepareSubmit() {
    if(isManual) {
        var manualVal = document.getElementById('stationManualInput').value.trim();
        if(!manualVal) { alert('Please enter a station name.'); return false; }
        document.getElementById('stationNameFinal').value = manualVal;
    } else {
        var dropVal = document.getElementById('stationDropdown').value;
        if(!dropVal) { alert('Please select a station or switch to manual entry.'); return false; }
        document.getElementById('stationNameFinal').value = dropVal;
    }
    return true;
}
</script>
