...
public class AddReadingServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            String sql = "INSERT INTO readings (station_name, location, state, rainfall_mm, water_level_m, " +
                        "risk_level, trend, officer_id, notes, latitude, longitude) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            if(result > 0) {
                // Send email notifications after successful insert asynchronously
                ...
                response.sendRedirect("officer-add-reading.jsp?success=true");
            } else {
                response.sendRedirect("officer-add-reading.jsp?error=true");
            }
        ...
        try {
            ...
            while(rs.next()) {
                ...
                if(isMatchingLocation) {
                    sendWarningEmail(userEmail, state, location, stationName, riskLevel, waterLevel, rainfall, trend);
                } else {
                    sendUpdateEmail(userEmail, state, location, stationName, riskLevel, waterLevel, rainfall, trend);
                }
            }
    ...
    private void sendWarningEmail(String toEmail, String state, String location, 
                                 String stationName, String riskLevel, 
                                 Double waterLevel, Double rainfall, String trend) {
        try {
            ...
            message.setSubject("⚠️ PERHATIAN: Bacaan Baru di Kawasan Anda - HydroAlert");
            ...
            String emailContent = "<!DOCTYPE html>" +
                ...
                "<div class='container'>" +
                "<div class='header'>" +
                "<h2>⚠️ PERHATIAN KAWASAN ANDA</h2>" +
                "<p>Bacaan Baru Telah Direkodkan</p>" +
                "</div>" +
                "<div class='content'>" +
                "<div class='alert-box'>" +
                "<strong>🔔 Amaran:</strong> Terdapat bacaan baru bagi kawasan anda (" + state + ")" +
                "</div>" +
                "<div class='details'>" +
                "<h3 style='color: #dc3545; margin-top: 0;'>Maklumat Bacaan</h3>" +
            ...
            emailContent += "</div>" +
                ...
                "<li>Sentiasa berwaspada dan pantau keadaan semasa</li>" +
                "<li>Ikuti arahan pihak berkuasa tempatan</li>" +
                "<li>Sediakan pelan kecemasan jika perlu</li>" +
                "<li>Jauhkan diri dari kawasan berisiko tinggi</li>" +
                "</ul>" +
                "</div>" +
                "</div>" +
                "<div class='footer'>" +
    ...
    private void sendUpdateEmail(String toEmail, String state, String location, 
                                String stationName, String riskLevel, 
                                Double waterLevel, Double rainfall, String trend) {
        try {
            Properties props = new Properties();
            ...
            message.setFrom(new InternetAddress(FROM_EMAIL));
            message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(toEmail));
            message.setSubject("📢 Kemaskini: Bacaan Baru Sistem HydroAlert");
            ...
            String emailContent = "<!DOCTYPE html>" +
                ...
                "<div class='container'>" +
                "<div class='header'>" +
                "<h2>📢 Kemaskini Bacaan</h2>" +
                "<p>Bacaan Baru Telah Direkodkan</p>" +
                "</div>" +
                "<div class='content'>" +
                "<div class='info-box'>" +
                "<strong>ℹ️ Maklumat:</strong> Bacaan baru untuk kawasan berikut telah direkodkan dalam sistem." +
                "</div>" +
                "<div class='details'>" +
                "<h3 style='color: #007bff; margin-top: 0;'>Maklumat Bacaan</h3>" +
                "<div class='detail-row'><span class='label'>Stesen:</span> " + stationName + "</div>" +
                "<div class='detail-row'><span class='label'>Lokasi:</span> " + location + ", " + state + "</div>" +
                "<div class='detail-row'><span class='label'>Tahap Risiko:</span> <span class='risk-" + riskLevel.toLowerCase() + "'>" + riskLevel + "</span></div>" +
                "<div class='detail-row'><span class='label'>Arah Aliran:</span> " + trend + "</div>";
            ...
            emailContent += "</div>" +
                "<div style='margin-top: 20px; padding: 15px; background-color: #fff3cd; border-left: 4px solid #856404; border-radius: 5px;'>" +
